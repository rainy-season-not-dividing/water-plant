...
export function buildHUDLabels(camera: Camera3D, telemetry: TelemetryState): Renderable[] {
  ...
  helperHUDLabel(20, -40, 10, '3# 超滤产水监测', `超滤产水浊度: ${telemetry.outletTurbidity.toFixed(3)} NTU`, '目标: <1 NTU', '#0ea5e9', camera, list);

  const colorUf = telemetry.ufPressure > 300 ? '#f43f5e' : '#38bdf8';
  const statUf = telemetry.ufPressure > 300 ? 'TMP 偏高，建议复核' : '在役稳态回收';
  helperHUDLabel(50, 220, 20, '4# 超滤 超滤膜组', `TMP: ${telemetry.ufPressure} kPa`, `工况: ${statUf}`, colorUf, camera, list);
  helperHUDLabel(280, 20, 60, '5# 一级反渗透 膜组', `产水 TDS: ${telemetry.roTds} mg/L`, '目标: 100-300 mg/L', '#10b981', camera, list);
